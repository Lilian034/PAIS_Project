...
let STAFF_PASSWORD = 'admin123456';
...
function getStaffPassword() {
    return localStorage.getItem('staff_password') || STAFF_PASSWORD;
}

// ==================== 統一 HTTP 請求處理 ====================

/**
 * 統一的 API 請求處理函數（DRY 原則）
 */
...
async function generateStaffContent(topic, style = "formal", length = "medium") {
    try {
        const data = await apiRequest(`${STAFF_API_URL}/content/generate`, {
            method: 'POST',
...
async function getTasks(limit = 50) {
    try {
        const data = await apiRequest(`${STAFF_API_URL}/content/tasks?limit=${limit}`, { requireAuth: true });
...
async function getTask(taskId) {
    try {
        const data = await apiRequest(`${STAFF_API_URL}/content/task/${taskId}`, { requireAuth: true });
...
async function updateTask(taskId, content, editor = 'admin') {
    try {
        const data = await apiRequest(`${STAFF_API_URL}/content/task/${taskId}`, {
            method: 'PUT',
            body: { content, editor },
            requireAuth: true
        });
...
async function approveTask(taskId) {
    try {
        const data = await apiRequest(`${STAFF_API_URL}/content/task/${taskId}/approve`, {
            method: 'POST',
            requireAuth: true
        });
        return { success: true, message: data.message };
...
async function generateVoice(taskId) {
    try {
        const data = await apiRequest(`${STAFF_API_URL}/media/voice/${taskId}`, {
            method: 'POST',
            requireAuth: true
        });
        return { success: true, ...data };
...
async function generateVideo(taskId, imagePath, prompt = null) {
    try {
        ...
        const data = await apiRequest(`${STAFF_API_URL}/media/video/${taskId}?${queryParams}`, {
            method: 'POST',
            requireAuth: true
        });
        return { success: true, ...data };
...
async function uploadFile(file, folder = '') {
    try {
        ...
        const data = await apiRequest(`${API_BASE_URL}/upload`, {
            method: 'POST',
            body: formData,
            requireAuth: true
        });
...
async function ingestDocuments(folderPath = "documents") {
    try {
        const data = await apiRequest(`${API_BASE_URL}/ingest`, {
            method: 'POST',
...
async function listDocuments() {
    try {
        const data = await apiRequest(`${API_BASE_URL}/documents`, { requireAuth: true });
...
async function deleteDocument(filePath) {
    try {
        const data = await apiRequest(`${API_BASE_URL}/documents/${encodeURIComponent(filePath)}`, {
            method: 'DELETE',
            requireAuth: true
        });
        return { success: true, ...data };
...
async function staffHealthCheck() {
    try {
        return await apiRequest('/api/staff-health');
    } catch (error) {
        return { status: 'error' };
    }
}